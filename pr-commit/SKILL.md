---
name: pr-commit
description: Creates a commit for PR review feedback fixes — runs tests and lint scoped to changed files, checks for stale rename terms and missing test coverage, then commits with a conventional-format message.
---
...
2.5. Detect and run the linter before committing. Look for the nearest linter, scoped to changed files only:
   ...
   - If lint errors are found, STOP and show them. Do NOT proceed with the commit.
...
10. Report back with the commit hash and message
